...
export class RoutingAPIPipeline extends Stack {
  constructor(scope: Construct, id: string, props?: StackProps) {
    ...
    const jsonRpcProvidersSecret = sm.Secret.fromSecretAttributes(this, 'RPCProviderUrls', {
      ...
      secretCompleteArn:
        'arn:aws:secretsmanager:ap-southeast-1:978958364414:secret:routing-api-rpc-urls-json-primary-ixS8mw',
      ...
      */
      //secretCompleteArn: arn:aws:secretsmanager:ap-southeast-1:978958364414:secret:routing-api-rpc-urls-json-backup-D2sWoe
    })

    const tenderlyCreds = sm.Secret.fromSecretAttributes(this, 'TenderlyCreds', {
      secretCompleteArn: 'arn:aws:secretsmanager:ap-southeast-1:978958364414:secret:tenderly-api-wQaI2R',
    })

    const ethGasStationInfoUrl = sm.Secret.fromSecretAttributes(this, 'ETHGasStationUrl', {
      secretCompleteArn: 'arn:aws:secretsmanager:ap-southeast-1:978958364414:secret:eth-gas-station-info-url-ulGncX',
    })

    const pinataApi = sm.Secret.fromSecretAttributes(this, 'PinataAPI', {
      secretCompleteArn: 'arn:aws:secretsmanager:ap-southeast-1:978958364414:secret:pinata-api-key-UVLAfM',
    })
    const route53Arn = sm.Secret.fromSecretAttributes(this, 'Route53Arn', {
      secretCompleteArn: 'arn:aws:secretsmanager:ap-southeast-1:978958364414:secret:Route53Arn-elRmmw',
    })

    const pinataSecret = sm.Secret.fromSecretAttributes(this, 'PinataSecret', {
      secretCompleteArn: 'arn:aws:secretsmanager:ap-southeast-1:978958364414:secret:pinata-secret-svGaPt',
    })

    const hostedZone = sm.Secret.fromSecretAttributes(this, 'HostedZone', {
      secretCompleteArn: 'arn:aws:secretsmanager:ap-southeast-1:978958364414:secret:hosted-zone-JmPDNV',
    })
    ...
    const betaUsEast2Stage = new RoutingAPIStage(this, 'beta-ap-southeast-1', {
      env: { account: '978958364414', region: 'ap-southeast-1' },
    ...
    const prodUsEast2Stage = new RoutingAPIStage(this, 'prod-ap-southeast-1', {
      ...
      provisionedConcurrency: 100,
      ethGasStationInfoUrl: ethGasStationInfoUrl.secretValue.toString(),
      chatbotSNSArn: 'arn:aws:sns:ap-southeast-1:978958364414:SlackChatbotTopic',
      stage: STAGE.PROD,
      route53Arn: route53Arn.secretValueFromJson('arn').toString(),
      pinata_key: pinataApi.secretValueFromJson('pinata-api-key').toString(),
      pinata_secret: pinataSecret.secretValueFromJson('secret').toString(),
    ...
    const slackChannel = chatbot.SlackChannelConfiguration.fromSlackChannelConfigurationArn(
      this,
      'SlackChannel',
      'arn:aws:chatbot::978958364414:chat-configuration/slack-channel/eng-ops-slack-chatbot'
    )
